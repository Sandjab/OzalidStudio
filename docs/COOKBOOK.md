...
> **Document d'archive.** Ce cookbook décrit la chaîne historique (scripts Python,
> atelier HTML) qui a précédé Ozalid Studio ; ses marches à suivre ne s'appliquent
> plus telles quelles. Il est conservé ici comme **référence prestataires** : les
> gabarits (formules de dos, fonds perdus, marges) et les pièges relevés chez chaque
> prestataire restent la source des constantes de `src-tauri/src/providers.rs`.
