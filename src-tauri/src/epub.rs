...
///
/// C'est la convention de nommage de Google Fonts, que suit `outils/polices.sh` :
...
mod tests {
    ...
    /// Les noms de fichiers réellement posés par `outils/polices.sh`, groupés par
    ...
    /// Les noms qu'`outils/polices.sh` va réellement chercher pour une famille.
    ///
    ...
    fn la_liste_des_fichiers_suit_le_script_qui_les_pose() {
        for famille in crate::interieur::POLICES_TEXTE {
            ...
            assert!(
                !poses.is_empty(),
                "{famille} : `outils/polices.sh` ne pose plus aucun fichier sous ce \
                 répertoire OFL. Le tableau de `fichiers` doit suivre le script."
            );
            assert_eq!(
                copie, poses,
                "{famille} : le tableau de `fichiers` ne dit plus ce que \
                 `outils/polices.sh` pose dans `fonts/`. C'est au tableau de suivre \
